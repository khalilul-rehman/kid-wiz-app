...
const WrapText = (text) => {
  ...
  for (const word of words) {
    if (currentLine.length + word.length + (currentLine.length > 0 ? 1 : 0) <= MAXIMUM_CHARACTERS) {
    ...
    <DashboardContainer>
      ...
      <Box sx={{ display: 'flex', gap: $({ size: 24 }), userSelect: 'none' }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          {
            selectedCategory &&
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', gap: $({ size: 4 }) }}>
              <Typography sx={{
                fontSize: $({ size: 18 }),
                fontWeight: '600',
                lineHeight: $({ size: 30 }),
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <DownArrowSlimIcon size={$({ size: 16, numeric: true })} color={colors.extra.grey3} />
              </Box>
            </Box>
          }

          {
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', gap: $({ size: 4 }) }}>
              <Typography sx={{
                ...
                lineHeight: $({ size: 30 }),
                color: colors.solids.black,
              }}>{selectedSubject.title}</Typography>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
          <Box sx={{ position: 'relative' }}>
            <Box sx={{
              ...
              alignItems: 'center',
              zIndex: 1,
            }}><LocationIcon size={$({ size: 40, numeric: true })} color={colors.extra.grey2} /></Box>

            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
              {
                !selectedCategory &&
                <Box ref={categoryWheelRef} sx={{
                  position: 'relative',
                  height: $({ size: 400 }),
                  width: $({ size: 400 }),
                  userSelect: 'none',
                          ...
                          <g>
                            {
                              segmentCategories.map((subject, index) => {
                                ...
                                const rotationAngle = (360 / wheelData.length) + ((360 / wheelData.length) * 2) + (wheelData.length === 4 ? 45 : 0)
                                ...
                                return (
                                  <g key={subject.id}>
                                    <circle
                                      cx={$({ size: rotatedX * SCALE_DOWN_FACTOR, numeric: true })}
                                      cy={$({ size: rotatedY * SCALE_DOWN_FACTOR, numeric: true })}
                                      r={`${$({ size: 30 * SCALE_DOWN_FACTOR, numeric: true })}px`}
                                      fill='white'
                                    />
                                    <image
                                      href={subject?.iconPath}
                                      x={($({ size: rotatedX, numeric: true }) - $({ size: 35 / 2, numeric: true })) * SCALE_DOWN_FACTOR}
                                      y={($({ size: rotatedY, numeric: true }) - $({ size: 35 / 2, numeric: true })) * SCALE_DOWN_FACTOR}
                                      height={$({ size: 35 * SCALE_DOWN_FACTOR, numeric: true })}
                                      width={$({ size: 35 * SCALE_DOWN_FACTOR, numeric: true })}
                                    />
                                  </g>
                                )
                              })
                  ...
                  <Box sx={{ filter: `drop-shadow(0 0 ${$({ size: 5 })} ${alpha(colors.solids.black, 0.1)})` }}>
                    <Pie
                      ...
                      arcLabelsComponent={({ datum }) => {
                        ...
                        const subject = segmentSubjects.find((subject) => subject.id === datum.data.id)

                        const x = SPINNER_POINTS({ segments: segmentSubjects.length })[1][0].x
                        const y = -SPINNER_POINTS({ segments: segmentSubjects.length })[1][0].y

                        const newX = x * Math.cos(theta) - y * Math.sin(theta)
                        const newY = x * Math.sin(theta) + y * Math.cos(theta)

                        const rotationAngle = (360 / segmentSubjects.length) + ((360 / segmentSubjects.length) * 2) + (segmentSubjects.length === 4 ? 45 : 0)
                        ...
                        return (
                          <g key={subject.id}>
                            <circle
                              cx={$({ size: rotatedX * SCALE_DOWN_FACTOR, numeric: true })}
                              cy={$({ size: rotatedY * SCALE_DOWN_FACTOR, numeric: true })}
                              r={`${$({ size: 40 * SCALE_DOWN_FACTOR, numeric: true })}px`}
                              fill='white'
                            />
                            <image
                              href={subject?.iconPath}
                              x={($({ size: rotatedX, numeric: true }) - $({ size: 45 / 2, numeric: true })) * SCALE_DOWN_FACTOR}
                              y={($({ size: rotatedY, numeric: true }) - $({ size: 45 / 2, numeric: true })) * SCALE_DOWN_FACTOR}
                              height={$({ size: 45 * SCALE_DOWN_FACTOR, numeric: true })}
                              width={$({ size: 45 * SCALE_DOWN_FACTOR, numeric: true })}
                            />
                          </g>
                        )
                      }}
              ...
              {
                selectedSubject?.topics && (!selectedTopic || (selectedCategory && selectedSubject)) &&
                <Box ref={topicWheelRef} sx={{
                  position: 'relative',
                  height: $({ size: 400 }),
                  width: $({ size: 400 }),
                  userSelect: 'none',
                  ...
                  <Box sx={{ filter: `drop-shadow(0 0 ${$({ size: 5 })} ${alpha(colors.solids.black, 0.1)})` }}>
                    <Pie
                      ...
                      arcLabelsComponent={({ datum }) => {
                        ...
                        const topic = segmentTopics.find((subject) => subject.id === datum.data.id)
                        const textSegments = WrapText(topic.name)

                        const x = SPINNER_POINTS({ segments: segmentTopics.length })[1][0].x
                        const y = -SPINNER_POINTS({ segments: segmentTopics.length })[1][0].y

                        const newX = x * Math.cos(theta) - y * Math.sin(theta)
                        const newY = x * Math.sin(theta) + y * Math.cos(theta)

                        const rotationAngle = (360 / segmentTopics.length) + ((360 / segmentTopics.length) * 2) + (segmentTopics.length === 4 ? 45 : 0)

                        const rotatedX = newX * Math.cos(rotationAngle * (Math.PI / 180)) - newY * Math.sin(rotationAngle * (Math.PI / 180))
                        const rotatedY = newX * Math.sin(rotationAngle * (Math.PI / 180)) + newY * Math.cos(rotationAngle * (Math.PI / 180))

                        const centerX = $({ size: rotatedX * SCALE_DOWN_FACTOR, numeric: true }) + 0 / 2
                        const centerY = $({ size: rotatedY * SCALE_DOWN_FACTOR, numeric: true }) + 0 / 2
                        const textRotation = ((360 / segmentTopics.length) / 2) + ((arc.index) * (360 / segmentTopics.length))

                        return (
                          <g
                            key={topic.id}
                            x={$({ size: rotatedX * SCALE_DOWN_FACTOR, numeric: true })}
                            y={$({ size: (rotatedY * SCALE_DOWN_FACTOR) + ADJUSTED_OFFSET, numeric: true })}>
                            {
                              textSegments.map((_, index) => {
                                ...
                                return (
                                  <text
                                    key={index * 1.5}
                                    x={$({ size: rotatedX * SCALE_DOWN_FACTOR, numeric: true })}
                                    y={$({ size: (rotatedY * SCALE_DOWN_FACTOR + (18 * index) - (18 * (textSegments.length - 1) / 2)) + ADJUSTED_OFFSET, numeric: true })}
                                    textAnchor='middle'
                                    dominantBaseline='central'
                                    style={{
                                      fontSize: $({ size: 18 * SCALE_DOWN_FACTOR }),
                                      fontWeight: '500',
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <CustomButton
          ...
          rightIcon={
            selectedTopic
              ? <RightArrowIcon size={$({ size: 24, numeric: true })} color={colors.white[800]} />
              : <RefreshIcon size={$({ size: 24, numeric: true })} color={colors.white[800]} />
          }
          onClick={() => {
            ...
            } else if (selectedCategory?.subjects && selectedSubject?.topics && !selectedTopic) {
              HandleSpinToChooseTopic()
            } else if (selectedCategory?.subjects && selectedSubject?.topics && selectedTopic) {
              if (window.location.pathname === ROUTES.PARENT.EXPLORE.INDEX) {
                navigate(ROUTES.PARENT.LEARN_SUBJECT.CHAT)
              } else if (window.location.pathname === ROUTES.CHILD.EXPLORE.INDEX) {
