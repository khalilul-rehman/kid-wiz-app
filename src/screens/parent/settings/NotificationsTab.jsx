...
const NotificationsTab = ({ topSectionHeight = 0 }) => {
  ...
  const [emailNotifications, setEmailNotifications] = React.useState([
    ...
    },
    {
      ...
      'description': 'Receive a weekly summary of your activity, right in your email inbox!',
      'isSelected': false
    },
  ])
            ...
            return (
              <Box sx={{
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                  <Typography sx={{ fontSize: $({ size: 18 }), fontWeight: '500', color: colors.solids.black }}>
                    {item.label}
                  </Typography>
                  <Typography sx={{ fontSize: $({ size: 13.5 }), fontWeight: '400', color: colors.solids.black }}>
                    {item.description}
                  </Typography>
                </Box>
              </Box>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: $({ size: 16 }) }}>
...

        {
          websiteNotifications.map((item, index) => {
            return (
              <Box key={index} sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: $({ size: 16 }) }}>
                <CustomCheckBox
